...
// import { HpBar, PokemonSprite, StatusIcon, TypeIcon } from '../../entities';
...
// import { BattleLog } from './objects/battle-log.obj';
// import { SideSections, SideData, OverlayButtonsData } from './battle.types';
...
//     animate: boolean
//   ) {
//     const { hp, hpLabel } = $side.sections;
//     await hp.setHp(pokemonData.hp / pokemonData.maxhp, animate, $side);
//     const hpLabelText =
//       $side.data.side === 'user'
...
//     pokemonData: Pokemon
//   ) {
//     const { pokemon, nickname, levelLabel, level, status } = $side.sections;
//     await pokemon.setPokemon(pokemonData.species, $side);
//     nickname.setText(pokemonData.name, $side);
//     this.updateHp($side, pokemonData, false);
...
//     const moveData = pokemonData.moves.map((move) => Dex.moves.get(toID(move)));
...
//     buttons.forEach((btn) => {
//       if (btn.gridPosition[0] === nextRow && btn.gridPosition[1] === nextCol) {
...
//         (move) =>
//           move.id === ($buttons.data.hoveredButton as MoveButton).moveData.id
//       )!;
...
//       await this.playTurn(
//         `move ${moveData.id}`,
//         `move ${this.battle.p2.pokemon[0].moveSlots[0].id}`
//       );
...
//   async playTurn(userChoice: string, foeChoice: string) {
//     this.battle.p1.choose(userChoice);
//     this.battle.p2.choose(foeChoice);
//     this.battle.commitDecisions();
//     await this.$log.logTurn(async (event) => {
//       const $side = event.side === 'user' ? this.$userSide : this.$foeSide;
//       const pokemon =
//         event.side === 'user'
//           ? this.battle.p1.active[0]
//           : this.battle.p2.active[0];
//       console.log(event);
//       switch (event.type) {
//         case 'move': {
//           await this.playMoveAnitmation();
//           break;
//         }
//         case 'damage':
//         case 'heal':
//           await this.updateHp($side, pokemon, true);
//           break;
//         case 'status':
//           $side.sections.status.setStatus(toID(pokemon.status), $side);
//           break;
//       }
//       await App.wait(500);
//     });
...
//   private async playMoveAnitmation() {}
// }
